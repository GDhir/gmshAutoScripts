...
{
public:
  Step3( const int quadratureOrder );

  void run( std::string filename );

private:
  void make_grid( std::string filename );
  void setup_system();
...
template<int dim>
Step3<dim>::Step3( const int quadratureOrder )
  : mapping(MappingFE<2>(FE_SimplexP<2>(1)), MappingFE<2>(FE_Q<2>(1)))
  , fe(FE_SimplexP<2>(2), FE_Q<2>(2))
  , quadrature_formula(QGaussSimplex<2>( quadratureOrder ), QGauss<2>( quadratureOrder ))
...
template<int dim>
void Step3<dim>::make_grid( std::string filename )
{
  std::cout << filename << "\n";
  GridIn<2>(triangulation).read_msh( filename );
...
// Nothing has changed here.
template <int dim>
void Step3<dim>::output_results(std::string meshFileName, std::string pythonVarName ) const
{

  std::string textfoldername{"/media/gaurav/easystore/dealii/MixedElement/TextFiles/"};

  std::regex regexVal("mesh|hanging",
...
// Nothing has changed here.
template<int dim>
void Step3<dim>::run( std::string filename, std::string pythonVarName )
{
  make_grid( filename );
  setup_system();
  assemble_system();
...
int main(int argc, char* argv[])
{
  ...
  std::string path = "/home/gaurav/Finch/src/examples/Mesh/MeshRun/mix_mesh";
  ...
  Step3<2> laplace_problem( quadratureOrder );
  laplace_problem.run( filename, pythonVarName );
