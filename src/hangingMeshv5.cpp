...
int main(int argc, char **argv)
{
    ...
    for( auto& Nx2: Nx2vals ) {
        ...
        std::vector< std::vector<int> > linesVec{ lines2, lines4, linesmid3 };
        // std::vector< std::vector<int> > linesVec{ lines1, lines2, lines3, linesmid2 };
        setTransfiniteCurves( linesVec );

        std::vector<int> planeIds{ p2, p4 };
        std::vector< std::vector<int> > ptsVec{ pts2, pts4 };
        std::vector<int> Nxvals{ Nx2, Nx4 };  
        std::vector<int> Nyvals{ Nright, Ny4 };
        ...
        std::string meshfilename = "hangingMeshv5Nx=" + std::to_string( Nx2 ) + "Ny=" + std::to_string( Ny ) + ".msh";
