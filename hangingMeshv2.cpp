...
int main(int argc, char **argv)
{
    ...
    gmsh::model::geo::mesh::setTransfiniteSurface(pl, "Left", {ptsleft[0], ptsleft[ (Nx - 1) ], ptsleft[ (Nx - 1) + (Ny - 1) ], ptsleft[ (Nx - 1)*2 + Ny - 1 ]});
    gmsh::model::geo::mesh::setTransfiniteSurface(pr, "Left", {ptsright[0], ptsright[ (Nx - 1) ], ptsright[ (Nx - 1) + (Nright - 1) ], ptsright[ (Nx - 1)*2 + (Nright - 1) ]});
// 
